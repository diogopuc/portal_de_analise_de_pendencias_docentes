...
import { SEMANAS_CONFIG } from '../../config/semanas.config';
...
function nomeMes(aba: string): string {
  ...
  return meses[aba.split('.')[1]] || aba;
}

export class PdfRelatorioGenerator {
  private readonly relatoriosDir: string;
  ...
  async gerarPDF(docente: Docente): Promise<{ nomeArquivo: string; caminho: string; tamanhoBytes: number }> {
    ...
    if (fs.existsSync(logoPath)) {
      try {
        ...
        page.drawImage(logoImg, { x: marginX, y: headerCenterY - logoH / 2, width: logoW, height: logoH });
    ...
    page.drawText(titleText,    { x: titleX,    y: headerCenterY + 8,  size: 11, font: fontBold,    color: WHITE });
    page.drawText(subtitleText, { x: subtitleX, y: headerCenterY - 10, size: 9,  font: fontRegular, color: rgb(220 / 255, 180 / 255, 200 / 255) });

    // SAUDAÇÃO
    y = height - HEADER_H - 20;
    const mesNome = nomeMes(docente.semanas[0]?.aba || '01.04');

    page.drawText(`Olá, Prof(a). ${docente.nomeDocente}.`, { x: marginX, y, size: 12, font: fontBold, color: PRIMARY });
    const nomeWidth = fontBold.widthOfTextAtSize(`Olá, Prof(a). ${docente.nomeDocente}.`, 12);
    page.drawText(`  |  Matrícula: ${docente.matricula}`, { x: marginX + nomeWidth, y: y + 1, size: 9, font: fontRegular, color: GRAY });
    y -= 16;

    page.drawText(`Segue o detalhamento das pendências ao longo do mês de ${mesNome}.`, { x: marginX, y, size: 10, font: fontRegular, color: DARK });
    y -= 10;
    ...
    boxesAtivos.forEach((box, i) => drawResumoBox(marginX + i * (boxWDinamico + 16), boxWDinamico, box.label));
    y -= BOX_H + 16;
    ...
    for (const config of SEMANAS_CONFIG) {
      ...
      if (!semana) continue;
      if (semana.abonada) continue;
      if (!semana.pendenciaAgenda && !semana.pendenciaTach) continue;

      const linhasTach = semana.pendenciaTach
        ? semana.statusPorDia.filter(sp => sp.status && !(STATUS_TACH_SEM_PENDENCIA as readonly string[]).includes(sp.status)).length
        : 0;
      const estimativa = 32 + (semana.pendenciaAgenda ? 38 : 0) + (semana.pendenciaTach ? 14 + linhasTach * 14 : 0) + 40;
      if (y < estimativa + 50) {
      ...
      page.drawText(`${config.semana}  —  ${config.periodo}`, { x: marginX + 10, y: y - TITLE_H + 8, size: 10, font: fontBold, color: WHITE });
      y -= TITLE_H + 8;
      ...
      if (semana.pendenciaTach && semana.statusPorDia.length > 0) {
        ...
        for (const sp of semana.statusPorDia) {
          if (!sp.status || (STATUS_TACH_SEM_PENDENCIA as readonly string[]).includes(sp.status)) continue;
          if (y < 70) { page = pdfDoc.addPage(PageSizes.A4); y = height - 40; }
          const statusExibido = mapearStatus(sp.status);
          const corStatus = (STATUS_TACH_COM_PENDENCIA as readonly string[]).includes(sp.status) ? WARNING_COLOR : ORANGE;
    ...
    pdfDoc.getPages().forEach(pg => {
      ...
      pg.drawText('PUCPR © 2026 — Portal de Análise de Pendências Docentes', { x: marginX, y: 18, size: 7, font: fontRegular, color: GRAY });
      pg.drawText(`Gerado em: ${new Date().toLocaleDateString('pt-BR')}`,      { x: width - 160, y: 18, size: 7, font: fontRegular, color: GRAY });
    });
